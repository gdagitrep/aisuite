...
from aisuite.framework.message import TranscriptionResult, Segment, Word, Alternative, Channel
...
class DeepgramProvider(Provider):
    ...
    def chat_completions_create(self, model, messages):
        ...
        raise NotImplementedError("Deepgram provider only supports audio transcription, not chat completions.")
    ...
    def audio_transcriptions_create(self, model: str, file: Union[str, BinaryIO], **kwargs) -> TranscriptionResult:
        ...
        try:
            ...
            if response.status_code != 200:
                raise ASRError(f"Deepgram API error: {response.status_code} - {response.text}")
        ...
        try:
            ...
            for chan_data in channels_data:
                ...
                for alt_data in chan_data.get("alternatives", []):
                    chan_alternatives.append(Alternative(
                        transcript=alt_data.get("transcript", ""),
                        confidence=alt_data.get("confidence", None)
                    ))
